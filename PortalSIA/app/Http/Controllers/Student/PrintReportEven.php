...
class PrintReportEven extends Controller
{
    public function CalculateFinalScore($NISN, $subject)
    {
        ...
        foreach(SubjectScore::query()->where('NISN', Auth::user()->NISN)->where('school_year_id', session('tempSchoolYear'))->whereHas('subject', fn($query) => $query->where('name', $subject))->get() as $data)
        {
    ...
    public function View()
    {
        ...
        $extracurricular = TakingExtracurricular::query()->where('NISN', Auth::user()->NISN)->where('school_year_id', session('currentSchoolYear'))->first()->extracurricullar->name ?? "";
        $extracurricularScore = ExtracurricularScore::query()->where('NISN', Auth::user()->NISN)->where('extracurricular_id', TakingExtracurricular::query()->where('NISN', Auth::user()->NISN)->where('school_year_id', session('currentSchoolYear'))->first()->extracurricular_id ?? "")->first()->score ?? "";
        ...
        $data = [
            ...
            'semester' => "Genap",
            'NISN' => Auth::user()->NISN,
            'class' => HomeroomClass::query()->where('NISN', Auth::user()->NISN)->where('school_year_id', session('currentSchoolYear'))->first(),
            ...
            'fiqihFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "FIQIH"),
            'SKIFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "SEJARAH KEBUDAYAAN ISLAM"),
            'PKNFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "PKN"),
            'bahasaIndonesiaFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "BAHASA INDONESIA"),
            'bahasaArabFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "BAHASA ARAB"),
            'bahasaInggrisFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "BAHASA INGGRIS"),
            'matematikaFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "MATEMATIKA"),
            'IPAFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "IPA"),
            'IPSFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "IPS"),
            'kesenianFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "KESENIAN"),
            'seniBudayaFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "SENI BUDAYA"),
            'PENJASFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "PENJAS"),
            'BPFinalScore' => $this->CalculateFinalScore(Auth::user()->NISN, "BP"),
